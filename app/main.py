...
from backtesting.phase_39_1_evidence_diagnostic import (
    Phase391EvidenceDiagnostic,
)
...
def run_phase_39(
    ...
    It audits the frozen Evidence Model against OOS data.
    It never changes DecisionEngine or BUY/SELL.
    """
    ...
    if hasattr(summary, "iterrows"):
        for _, row in summary.iterrows():
            ...
            score = row.get("score")
            if horizon is not None and score is not None:
                current_scores[str(horizon)] = safe_float(score)

    validator = Phase39StatisticalValidation(
    ...
    print(
        "Phase 3.9 remains research/validation only."
    )
...
def run_phase_391(
    ...
    """
    Phase 3.9.1 investigates evidence-model behavior.

    It does not retrain, reweight, or alter production decisions.
    """
    ...
    diagnostic_engine = Phase391EvidenceDiagnostic(
        evidence_engine=evidence_engine,
        test_data=test_data,
    ...
    results = diagnostic_engine.run()

    diagnostic_engine.print_report(results)
    ...
    print(
        "Phase 3.9.1 classification: "
        f"{diagnosis.get('classification', 'UNKNOWN')}"
    )

    print(
        "Phase 3.9.1 remains research/validation only."
    )
...
def main():
    ...
    # ======================================================
    # PHASE 3.8 -> EVIDENCE MODEL HANDOFF
    # ======================================================
    ...
    if robustness_results is not None:
        handoff = evidence_engine.apply_phase_38_robustness(
            robustness_results
        )

        robust_total = sum(
            len(features)
            for features in handoff.values()
        )
        ...
        print(
            f"ROBUST features admitted: {robust_total}"
        )
    else:
        handoff = evidence_engine.apply_phase_38_robustness(
            None
    ...
    # Re-evaluate current/test evidence only after the
    # authoritative Phase 3.8 feature gate has been applied.
    current_evidence, evidence_test_results, evidence_statistics = (
        evaluate_evidence(
            evidence_engine,
            current_features,
            test_data,
    ...
    # ======================================================
    # 11. PHASE 3.9 STATISTICAL VALIDATION
    # ======================================================
    ...
    # ======================================================
    # 12. PHASE 3.9.1 DIAGNOSTIC
    # ======================================================

    phase391_scores = {}

    if hasattr(current_summary, "iterrows"):
        for _, row in current_summary.iterrows():
            horizon = row.get("horizon")
            score = row.get("score")
            if horizon is not None and score is not None:
                phase391_scores[str(horizon)] = safe_float(score)
