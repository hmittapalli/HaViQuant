...
def render_company_intelligence(ticker: str):
    st.markdown("<div class='hq-page-kicker'>360° Company Intelligence · Evidence First</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='hq-page-title'>{ticker.upper()} <span style='color:#4b6b86;'>/ Company Intelligence</span></div>", unsafe_allow_html=True)
    st.markdown("<div class='hq-page-subtitle'>Company history, business model, products, demand, financial history, earnings, backlog, competition, governance, scenarios and stock-level context. Missing data stays missing.</div>", unsafe_allow_html=True)

    c1, c2 = st.columns([1, 2])
    with c1:
        quarters = st.selectbox("Historical quarters", [4, 6, 8, 10, 12, 20], index=3, key="company_quarters")
    with c2:
        competitors_csv = st.text_input("Competitors (optional, comma-separated)", value="", placeholder="AMD, AVGO, MSFT", key="company_competitors")

    try:
        data = load_company_intelligence(ticker, int(quarters), competitors_csv)
    except Exception as exc:
        st.error(f"Company Intelligence could not load for {ticker}.")
        st.exception(exc)
        return
    if not data.get("available"):
        st.error(data.get("error", "Company data unavailable."))
        return
    ...
    st.markdown(f"**{profile['name']}** · {profile['sector']} · {profile['industry']} · {profile['country']}")
    st.caption(f"Data snapshot: {data['as_of']} · {data['sources']['market_data']}")

    cards = st.columns(6)
    cards[0].metric("Business Quality", f"{scores['business_quality']}/100" if scores.get('business_quality') is not None else "N/A")
    cards[1].metric("Financial Strength", f"{scores['financial_strength']}/100" if scores.get('financial_strength') is not None else "N/A")
    cards[2].metric("Demand Proxy", f"{scores['demand_proxy']}/100" if scores.get('demand_proxy') is not None else "N/A")
    cards[3].metric("Current Price", f"${val['current_price']:,.2f}" if val.get('current_price') else "N/A")
    cards[4].metric("Reference Upside", _fmt_pct(val.get('reference_upside_pct')))
    cards[5].metric("Beta", _fmt_number(stock.get('beta'), 2))

    tabs = st.tabs(["🏢 Business", "📊 Quarters", "🔥 Demand Timeline", "📦 Orders & Backlog", "🆚 Competition", "⚖️ Ethics & Risk", "📅 Earnings", "💎 Valuation", "🧩 Stock Level", "🧪 Evidence"] )

    with tabs[0]:
        st.subheader("Business Model")
        st.write(profile.get("summary") or "Business summary unavailable.")
        a, b, c, d = st.columns(4)
        a.metric("Sector", profile.get("sector", "N/A"))
        b.metric("Industry", profile.get("industry", "N/A"))
        c.metric("Employees", f"{int(profile['employees']):,}" if profile.get('employees') else "N/A")
        d.metric("Market Cap", _fmt_money(profile.get("market_cap")))
        st.info("Products, customers, partners and suppliers should be verified from company filings and investor-relations disclosures before being treated as confirmed facts.")

    with tabs[1]:
        st.subheader(f"Last {data['quarter_count']} Reported Quarters")
        qrows = data.get("quarters", [])
        if qrows:
            qdf = pd.DataFrame(qrows)
            for col in ["revenue", "gross_profit", "operating_income", "net_income", "free_cash_flow"]:
                if col in qdf.columns:
                    qdf[col] = qdf[col].map(_fmt_money)
            for col in ["revenue_yoy_pct", "revenue_qoq_pct", "gross_margin_pct", "operating_margin_pct"]:
                if col in qdf.columns:
                    qdf[col] = qdf[col].map(lambda x: _fmt_pct(x))
            qdf = qdf.rename(columns={"quarter":"Quarter","revenue":"Revenue","revenue_yoy_pct":"Revenue YoY","revenue_qoq_pct":"Revenue QoQ","gross_profit":"Gross Profit","gross_margin_pct":"Gross Margin","operating_income":"Operating Income","operating_margin_pct":"Operating Margin","net_income":"Net Income","eps":"EPS","free_cash_flow":"Free Cash Flow"})
            st.dataframe(qdf, width="stretch", hide_index=True)
            st.caption("Quarterly data comes from the market-data provider. Some companies expose fewer quarters or different accounting fields.")
        else:
            st.warning("No quarterly financial data was returned.")

    with tabs[2]:
        st.subheader("Current + Future Demand")
        st.metric("Current demand proxy", data["products_demand"].get("current_demand_proxy", "N/A"))
        for d in data["products_demand"].get("future_demand", []):
            icon = "🟢" if d["status"] == "POSITIVE" else "🔴" if d["status"] == "NEGATIVE" else "🟡"
            st.markdown(f"**{icon} {d['driver']}** · {d['status']} · {d['confidence']} confidence")
            st.caption(f"{d['value']} · Timeline: {d['timeline']}")
        st.warning("Future-demand statements are proxies unless supported by company guidance, customer orders, industry data or other cited evidence. HaViQuant does not convert them into guaranteed forecasts.")

    with tabs[3]:
        st.subheader("Orders, Backlog & Revenue Visibility")
        b = data["backlog"]
        st.metric("Backlog", "N/A")
        st.warning(b["status"] + ".")
        st.write(b["note"])
        st.markdown("**Required evidence for a company-specific backlog model:** orders/bookings, backlog or RPO definition, cancellation terms, expected conversion, book-to-bill where applicable, and customer concentration.")
        ...
        if rows:
            ...
            for col in ["Market Cap"]:
                if col in cdf.columns:
                    cdf[col] = cdf[col].map(_fmt_money)
            for col in ["Revenue Growth", "Gross Margin"]:
                if col in cdf.columns:
                    cdf[col] = cdf[col].map(lambda x: _fmt_pct(x))
            st.dataframe(cdf, width="stretch", hide_index=True)
            st.caption("Competitors are user-supplied in this version. Backlog is not compared unless the underlying source definitions are available and normalized.")
        else:
            st.info("Enter competitor tickers above to build a comparable market-data table.")

    with tabs[5]:
        st.subheader("Ethics, Governance & Risk Review")
        st.caption("A review queue is safer than inventing allegations. Items below require evidence verification.")
        for item in data["governance_ethics"]:
            icon = "🟢" if item["status"] == "AVAILABLE" else "🟡"
            st.markdown(f"**{icon} {item['topic']}** · {item['status']}")
            st.caption(item["evidence"])
        st.markdown("### Current risk flags")
        for r in data["risks"]:
            icon = "🔴" if r["level"] == "HIGH" else "🟠" if r["level"] == "MEDIUM" else "🟢"
            st.markdown(f"**{icon} {r['title']}** — {r['detail']}")
        if not data["risks"]:
            st.info("No automated risk flag was triggered from the available provider data. This is not proof that no risk exists.")

    with tabs[6]:
        st.subheader("Earnings & Catalyst Timing")
        e = data["earnings"]
        st.metric("Next earnings", str(e.get("next_earnings", "N/A")))
        st.metric("Last fiscal year end", str(e.get("last_earnings", "N/A")))
        st.caption("Earnings dates are provider data and can change. Verify against the company's investor-relations calendar before trading around the event.")

    with tabs[7]:
        st.subheader("Valuation Reference")
        v1, v2, v3, v4 = st.columns(4)
        v1.metric("Forward P/E", _fmt_number(val.get("forward_pe"), 2))
        v2.metric("Trailing P/E", _fmt_number(val.get("trailing_pe"), 2))
        v3.metric("P/S", _fmt_number(val.get("price_to_sales"), 2))
        v4.metric("P/B", _fmt_number(val.get("price_to_book"), 2))
        st.info(val["note"])
        if val.get("target_mean") is not None:
            st.metric("Analyst reference target", f"${val['target_mean']:,.2f}")

    with tabs[8]:
        st.subheader("Stock-Level Context")
        a,b,c,d = st.columns(4)
        a.metric("52W Low", f"${stock['fifty_two_week_low']:,.2f}" if stock.get('fifty_two_week_low') else "N/A")
        b.metric("52W High", f"${stock['fifty_two_week_high']:,.2f}" if stock.get('fifty_two_week_high') else "N/A")
        c.metric("Avg Volume", f"{stock['average_volume']:,.0f}" if stock.get('average_volume') else "N/A")
        d.metric("Shares", f"{stock['shares_outstanding']:,.0f}" if stock.get('shares_outstanding') else "N/A")
        st.info("Technical BUY/SELL decisions remain in the existing Decision Engine. Company Intelligence does not override the production signal.")

    with tabs[9]:
        st.subheader("Evidence & Completeness")
        checklist = [
            ("Company identity", bool(profile.get("name"))),
            ("Quarterly financials", bool(data.get("quarters"))),
            ("Earnings timing", bool(data["earnings"].get("next_earnings"))),
            ("Demand proxy", bool(data["products_demand"].get("future_demand"))),
            ("Backlog source", data["backlog"]["status"] == "AVAILABLE"),
            ("Governance review", bool(data.get("governance_ethics"))),
            ("Competitor comparison", bool(data.get("competition"))),
        ]
        for label, ok in checklist:
            st.markdown(f"{'🟢' if ok else '🟡'} **{label}**")
        st.caption("A green item means the module returned data, not that the underlying investment conclusion is correct. Missing information is explicitly surfaced rather than guessed.")
