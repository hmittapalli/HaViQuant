...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        return page, st.session_state.ticker
...
def render_live_news(
    ...
    """
    Render ticker-specific live news.
    ...
    News is display-only in this phase.
    """
    ...
    st.caption(
        f"Recent {ticker.upper()} headlines. "
        "News does not affect the price chart or BUY / SELL decision."
    )

    with st.spinner(
        f"Loading {ticker.upper()} news..."
    ):
    ...
    if not news:
        ...
        st.info(
            f"No live news was returned for {ticker.upper()} right now."
        )
...
def main():
    ...
    # Live Market News
    #
    # Independent display-only intelligence.
    # Does NOT affect chart or Decision Engine.
    # --------------------------------------------------------
    ...
    st.caption(
        "HaViQuant · Turn Market Data Into Decisions · Evidence Model research-only"
    )
